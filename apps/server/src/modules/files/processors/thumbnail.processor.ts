...
export class ThumbnailProcessor {
  ...
  async handleThumbnailGeneration(job: Job<ThumbnailJobData>): Promise<void> {
    ...
    try {
      // 检查缩略图功能是否启用
      const thumbnailEnabled = this.appConfig.app.file.thumbnailEnabled;
      if (!thumbnailEnabled) {
      ...
      // 生成缩略图URL
      const domain = this.appConfig.app.file.domain;
  ...
  private async generateImageThumbnail(inputPath: string, uploadId: string): Promise<string> {
    ...
    const uploadDir = this.appConfig.app.file.location;
