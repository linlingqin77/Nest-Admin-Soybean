...
export class VersionService {
  ...
  async checkAndCleanOldVersions(parentFileId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `清理旧版本失败: ${error instanceof Error ? error.message : String(error)}`,
  ...
  private async deleteVersion(version: FileVersion): Promise<void> {
    try {
      ...
      this.logger.error(
        `删除版本失败: ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error.stack : String(error),
      );
    }
  }
  ...
  async deletePhysicalFile(file: FileVersion): Promise<void> {
    try {
      if (file.storageType === 'local') {
        // 删除本地文件
        const baseDir = path.join(process.cwd(), this.appConfig.app.file.location);

        // 从URL中提取文件路径
        const url = file.url;
        const serveRoot = this.appConfig.app.file.serveRoot;
      ...
      this.logger.error(
        `删除物理文件失败: ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error.stack : String(error),
      );
    }
  }
