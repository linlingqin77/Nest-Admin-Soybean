...
export class UploadService {
  ...
  private cos = new COS({
    // 必选参数
    SecretId: this.appConfig.cos.secretId,
    SecretKey: this.appConfig.cos.secretKey,
    //可选参数
  ...
  constructor(
    ...
    this.thunkDir = 'thunk';
    this.isLocal = this.appConfig.app.file.isLocal;
  }
  ...
  async singleFileUpload(file: Express.Multer.File, folderId?: number) {
    ...
    const maxSize = this.appConfig.app.file.maxSize;
    ...
    if (this.isLocal) {
      ...
      const targetDir = this.appConfig.cos.location;
    ...
    if (res.filePath || this.isLocal) {
      const filePath = res.filePath || path.join(process.cwd(), this.appConfig.app.file.location, res.newFileName);
  ...
  private async getConfigValue(key: string, defaultValue: string): Promise<string> {
    try {
      const cfg = await this.prisma.sysConfig.findFirst({
        where: { configKey: key, delFlag: '0' },
      });
      return cfg?.configValue || defaultValue;
    } catch (error) {
  ...
  async chunkMergeFile(body: ChunkMergeFileDto) {
    ...
    const fileServePath = path.posix.join(this.appConfig.app.file.serveRoot, relativeFilePath);
    // 使用字符串拼接生成URL
    let domain = this.appConfig.app.file.domain;
    if (domain.endsWith('/')) {
    ...
    if (!this.isLocal) {
      ...
      // 使用字符串拼接生成COS URL
      let cosDomain = this.appConfig.cos.domain;
  ...
  async saveFileLocal(file: Express.Multer.File) {
    ...
    //文件服务完整路径
    const fileName = path.posix.join(this.appConfig.app.file.serveRoot, relativeFilePath);
  ...
  async uploadCos(targetFile: string, buffer: COS.UploadBody) {
    ...
    if (statusCode !== 200) {
      ...
      const data = await this.cos.putObject({
        Bucket: this.appConfig.cos.bucket,
        Region: this.appConfig.cos.region,
        Key: targetFile,
        Body: buffer,
      });
