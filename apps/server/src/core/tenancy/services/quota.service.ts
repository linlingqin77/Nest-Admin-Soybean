...
export class TenantQuotaService {
  ...
  async incrementUsage(tenantId: string, resource: QuotaResource, amount: number = 1): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to increment ${resource} usage for tenant ${tenantId}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async updateStorageUsage(tenantId: string, storageMB: number): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to update storage usage for tenant ${tenantId}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async syncUserCount(tenantId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to sync user count for tenant ${tenantId}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async clearQuotaCache(tenantId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to clear quota cache for tenant ${tenantId}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
