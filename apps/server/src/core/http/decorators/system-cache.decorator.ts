...
export function SystemCacheable(options: { key: string | ((args: unknown[]) => string); ttl?: number }) {
  return function <T extends object>(target: T, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (
      this: T & { redisService?: RedisService; moduleRef?: { get: (type: typeof RedisService) => RedisService } },
      ...args: unknown[]
    ) {
      // 获取 RedisService 实例
      ...
      try {
        ...
        // 缓存读取失败，继续执行原方法
        StaticLogger.warn(`Cache read error for key ${fullKey}: ${error}`, 'SystemCacheable');
      }
      ...
      if (result !== null && result !== undefined) {
        try {
          ...
          // 缓存写入失败不影响业务逻辑
          StaticLogger.warn(`Cache write error for key ${fullKey}: ${error}`, 'SystemCacheable');
        }
      }
      ...
      if (redisService) {
        for (const key of keys) {
          try {
            ...
            StaticLogger.warn(`Cache clear error for key ${key}: ${error}`, 'ClearSystemCache');
          }
        }
      }
