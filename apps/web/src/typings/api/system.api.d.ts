...
declare namespace Api {
  ...
  namespace System {
    ...
    /** role data scope params */
    type RoleDataScopeParams = CommonType.RecordNullable<{
      roleId: CommonType.IdType;
      dataScope: DataScope;
      deptIds: CommonType.IdType[];
    }>;

    /** role user search params */
    type RoleUserSearchParams = CommonType.RecordNullable<{
      roleId: CommonType.IdType;
      userName?: string;
      phonenumber?: string;
    }> &
    ...
    /** user profile update params (alias) */
    type UserProfileUpdateParams = UserProfileOperateParams;
    ...
    namespace FileManager {
      type RecycleFile = Api.System.FileInfo;
      type RecycleSearchParams = CommonType.RecordNullable<{ fileName?: string }> & Api.Common.CommonSearchParams;
      ...
      type FileVersions = { currentVersion: number; versions: FileVersion[] };
      type RestoreVersionParams = { fileId: string; targetVersionId: string };
      type RestoreVersionResult = { newVersion: number; uploadId: string };
      type FileAccessToken = { token: string; expiresIn: number };
      type StorageStats = { used: number; quota: number; percentage: number; remaining: number; companyName: string };
      type FolderSearchParams = Api.System.FolderSearchParams;
      type FolderOperateParams = Api.System.FolderOperateParams;
      type FolderList = Api.System.FolderList;
